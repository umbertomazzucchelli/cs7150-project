...
DATASET_PARAMS = {
    ...
    "WISDM": {
        # From Kwapisz et al. (2010)
        "LABELS": ["Walking", "Jogging", "Upstairs", "Downstairs", "Sitting", "Standing"],
        "N_CLASSES": 6,
        "data_file_pattern": "data/combined/WISDM_data.npy",  # Location is not used
    },
}
...
##################################################
COLUMN_NAMES = ["user", "activity", "timestamp", "x-axis", "y-axis", "z-axis"]  # Less relevant now
...
N_FOLDS = 10  # Default for KFold, LOSO determined by data

# Mapping (only relevant for R1 data loading logic)
LABELS_MAPPING_R1 = {
    "Sitting": 0,
    "Standing": 1,
    "Walking": 2,
    "Walking_Up_Stairs": 3,
    "Walking_Down_Stairs": 4,
    "Biking": 5,
    "Gym_Exercises": 6,
    "Lying_Down": 7,
}
...
##################################################
def parse_arguments():
    parser = argparse.ArgumentParser(description="Human Activity Recognition using PyTorch")
    parser.add_argument(
        "-m",
        "--model",
        type=str,
        choices=["lstm", "cnn"],
        default="lstm",
        help="Model architecture to use: lstm (default) or cnn",
    )
    parser.add_argument(
        "--train_full",
        action="store_true",
...
##################################################
### MODEL CLASSES (LSTM_HAR, CNN_LSTM_HAR - Unchanged)
##################################################
...
class CNN_LSTM_HAR(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, num_classes, dropout, n_time_steps):
        ...
        self.cnn_layers = nn.Sequential(
            nn.Conv1d(in_channels=input_size, out_channels=32, kernel_size=5, stride=1, padding=2),
            nn.BatchNorm1d(32),
            nn.ReLU(),
        ...
        self.cnn_output_size = self._get_cnn_output_size(n_time_steps)
        self.dense = nn.Sequential(nn.Linear(128 * self.cnn_output_size, hidden_size), nn.ReLU(), nn.Dropout(dropout))
        self.lstm = nn.LSTM(
...
##################################################
### TRAINING FUNCTION (Unchanged, uses global N_CLASSES)
##################################################
...
##################################################
def train_evaluate_fold(X_train, y_train, X_val, y_val, X_test, y_test, model_type):
    """
    Train and evaluate a model for a single fold
    """
    ...
    # if len(unique_classes) != N_CLASSES: # Less strict check, compute weights only for present classes
    #     print(f"Warning: Found {len(unique_classes)} classes in training data fold, expected {N_CLASSES}")
    #     print(f"Classes present: {unique_classes}")

    class_weights = np.zeros(N_CLASSES)
    # Compute weights only for classes present in the training fold
    ...
    if np.any(present_classes_mask):
        ...
        for idx, class_idx in enumerate(unique_classes):
            if class_idx < N_CLASSES:  # Safety check
    ...
    if model_type == "cnn":
        model = CNN_LSTM_HAR(
            input_size=N_FEATURES, hidden_size=N_HIDDEN_NEURONS, num_layers=N_LSTM_LAYERS, num_classes=N_CLASSES, dropout=DROPOUT, n_time_steps=SEGMENT_TIME_SIZE
        ).to(DEVICE)
    else:  # lstm
        model = LSTM_HAR(input_size=N_FEATURES, hidden_size=N_HIDDEN_NEURONS, num_layers=N_LSTM_LAYERS, num_classes=N_CLASSES, dropout=DROPOUT).to(DEVICE)
    ...
    return history, accuracy, f1, confusion_mat, model, scaler  # Return model/scaler for full training
...
def create_logs_directory():
    # Now creates logs/DATASET_NAME/LOCATION structure
...
def save_training_logs(model_type, history, results, confusion_matrix, timestamp, all_accuracies, all_f1_scores, all_histories, n_folds, cv_type="LOSO"):
    log_dir = create_logs_directory()  # Use the function to get the correct path
    # Construct filename including dataset and location
    log_file_base = f"training_log_{timestamp}_{model_type}_{DATASET_NAME}_{LOCATION}"
    ...
    log_data = {
        ...
        "hyperparameters": {
            ...
            "n_lstm_layers": N_LSTM_LAYERS if model_type == "lstm" or model_type == "cnn" else None,
            "n_epochs": N_EPOCHS,
            "l2_loss": L2_LOSS,
            "learning_rate": LEARNING_RATE,
            "dropout": DROPOUT,
            "segment_time_size": SEGMENT_TIME_SIZE,
            "n_hidden_neurons": N_HIDDEN_NEURONS if model_type == "lstm" or model_type == "cnn" else None,
...
if __name__ == "__main__":
    ...
    print(f"Using {args.model.upper()} model architecture on {DATASET_NAME} dataset")
    ...
    # Basic checks (already done in preprocessing, but good safety net)
    data = data[~np.isnan(data).any(axis=1)]
    # Ensure labels are within the expected range for the dataset
    ...
    for user_id in subjects:
        # Optional: Skip specific users if needed (e.g., user_id == 10 in R1)
        # if DATASET_NAME == "R1" and user_id == 10: continue
        user_data = data[data[:, 0] == user_id]
        # Windowing uses columns 1, 2, 3 (x, y, z)
        # Label is in the last column (-1)
    ...
    if args.train_full:
        ...
        try:
            ...
            print("Stratified split failed (likely too few samples per class), using non-stratified split.")
            X_train, X_val, y_train, y_val = train_test_split(X, y, test_size=0.1, random_state=RANDOM_SEED)

        print(f"Training set size: {len(X_train)}, Validation set size: {len(X_val)}")

        # Train the final model (using train_evaluate_fold but without test data)
        # Need to instantiate model, scaler, criterion, optimizer etc. here
        scaler = StandardScaler()
        X_train = scaler.fit_transform(X_train.reshape(-1, X_train.shape[-1])).reshape(X_train.shape)
        X_val = scaler.transform(X_val.reshape(-1, X_val.shape[-1])).reshape(X_val.shape)
        ...
        if args.cv_type == "loso":
            ...
            for fold, subject_to_leave_out in tqdm(enumerate(unique_subjects, 1), total=n_folds_cv, desc="CV Progress"):
                ...
                # Ensure validation split is possible and stratified if labels allow
                if len(X_train_val) > 1:
                    ...
                    X_val, y_val = X_train_val, y_train_val  # Use same data if split not possible

                # Check if train/val/test sets are empty after splitting
                if len(X_train) == 0 or len(X_val) == 0 or len(X_test) == 0:
                    print(f"Skipping fold {fold} due to empty train/val/test set after split.")
                    continue

                fold_history, fold_accuracy, fold_f1, fold_confusion_matrix, _, _ = train_evaluate_fold(X_train, y_train, X_val, y_val, X_test, y_test, args.model)
            ...
            # Use StratifiedKFold for potentially better class balance across folds
            skf = StratifiedKFold(n_splits=n_folds_cv, shuffle=True, random_state=RANDOM_SEED)

            # Need subject IDs if we want to do GroupKFold, but for standard KFold/StratifiedKFold, split X, y
        ...
        # Calculate and display average results (only if folds were actually run)
        if not all_accuracies:
            ...
            save_training_logs(
                args.model,
                all_histories[-1] if all_histories else None,  # Pass last history if available
                results,
            ...
            # Ensure normalization handles rows with zero sum
            ...
            plt.title(f"{args.model.upper()} Avg CM ({cv_type_str} - {DATASET_NAME} - {LOCATION})", fontsize=14)
            plt.ylabel("True label", fontsize=12)
            plt.xlabel("Predicted label", fontsize=12)
            plt.xticks(rotation=45, ha="right")
            plt.yticks(rotation=0)
            plt.tight_layout()
            plt.savefig(os.path.join(log_dir, f"confusion_matrix_{timestamp}_{args.model}_{DATASET_NAME}_{LOCATION}.png"))
            plt.close()
